...
TEST(test_attr_kernel, set_weights1) {
  AttrKernel attr_kernel("attr");
  ...
  attr_kernel.set_weights(weights);
  const auto &weights_ = attr_kernel.weights();
...
TEST(test_attr_kernel, set_bias1) {
  AttrKernel attr_kernel("attr");
  ...
  attr_kernel.set_bias(biases);
  const auto &biases_ = attr_kernel.bias();
...
TEST(test_attr_kernel, set_weights2) {
  AttrKernel attr_kernel("attr");
  ...
  attr_kernel.set_weights(weights_);
  attr_kernel.set_weights(weight_vals);

  weights_ = attr_kernel.weights();
...
TEST(test_attr_kernel, set_bias2) {
  AttrKernel attr_kernel("attr");
  uint32_t val_count = 9;
  ...
  attr_kernel.set_bias(biases_);
  attr_kernel.set_bias(bias_vals);

  biases_ = attr_kernel.bias();
...
TEST(test_attr_kernel, init_weights) {
  AttrKernel attr_kernel("attr");
  attr_kernel.InitWeights(3, 64, 32, 32);
  const std::vector<sftensor> weights = attr_kernel.weights();
...
TEST(test_attr_kernel, init_bias) {
  AttrKernel attr_kernel("attr");
  attr_kernel.InitBias(3, 64, 1, 1);
  const std::vector<sftensor> bias = attr_kernel.bias();
