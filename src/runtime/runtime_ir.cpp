...
RuntimeGraph::RuntimeGraph(std::string param_path, std::string bin_path)
    : param_path_(std::move(param_path)), bin_path_(std::move(bin_path)), graph_state_(GraphState::NeedInit) {}
...
bool RuntimeGraph::Init() {
  ...
  if (load_result != 0) {
    LOG(ERROR) << "Load param file path and bin file path error: " << param_path_ << " "
               << bin_path_;
    return false;
  }
...
void RuntimeGraph::Build(const std::string& input_name,
  ...
  CHECK(!this->ops_.empty())
      << "Graph operators are empty, may be no init";
  ...
  for (const auto& op : this->ops_) {
    if (op->type == "pnnx.Input") {
      ...
      slayer layer = RuntimeGraph::CreateLayer(op);
      CHECK(layer != nullptr) << "Layer create failed!";
      op->layer = layer;
      layer->set_runtime_op(op);
    }
  }
...
std::vector<sftensor> RuntimeGraph::Forward(const std::vector<sftensor>& inputs, bool debug) {
  // 检查计算图是否构建完毕
  if (graph_state_ < GraphState::Complete) {
  ...
  while (!ops_que.empty()) {
    ...
    // 当前节点不是输入节点，首先检测是否就绪，若就绪则执行
      ...
      // 执行当前节点
      InferStatus status = cur_op->layer->Forward();
      ...
      if (debug) {
        ...
        if (run_dur_infos.find(cur_op->type) ==
            run_dur_infos.end()) {
      ...
      ProbeNextOp(cur_op, ops_que, cur_op->out_oprands->data);
...
void RuntimeGraph::InitOpInputs(
  ...
  for (const pnnx::Operand* input : inputs) {
    ...
    const pnnx::Operator* producer = input->producer;  // 获取输入操作数的生产节点
    srunoprand oprand = std::make_shared<RuntimeOperand>();
    // 初始化输入操作数的名称、维度
    // 注意：输入操作数名称是其生产节点名称
...
void RuntimeGraph::InitOpOutputs(
...
void RuntimeGraph::ProbeNextOp(const srunop& cur_op, std::deque<srunop>& ops_que,
  ...
  for (const auto& [next_name, next_op] : next_ops) {
    ...
    if (next_in_oprands.find(cur_op->name) !=
        next_in_oprands.end()) {
