...
void RuntimeOperatorUtils::InitOpsOutput(
  ...
  for (uint32_t i = 0; i < pnnx_ops.size(); ++i) {
    ...
    CHECK(pout_oprands.size() == 1) << "One operator has <= one output oprand in TinyInfer";
    ...
    const auto& out_oprand = op->out_oprands;
      ...
      for (int b = 0; b < batch; ++b) {
        if (out_shape.size() == 2) {
          ...
          tmp_out_oprand->data.push_back(std::make_shared<ftensor>(
              1, out_shape.at(1), out_shape.at(2)));
        } else {  // current shape size is 4
          tmp_out_oprand->data.push_back(std::make_shared<ftensor>(
              out_shape.at(1), out_shape.at(2),
              out_shape.at(3)));
        }
      }
      op->out_oprands = std::move(tmp_out_oprand);
    } 
      ...
      for (uint32_t b = 0; b < batch; ++b) {
        ...
        if (out_shape.size() == 2) {
          if (tensor_shape.at(0) != 1 ||
              ...
              tensor_shape.at(2) != 1) {
            DLOG(WARNING)
                << "The shape of output tensor do not adapting with output oprand shape";
          ...
          if (tensor_shape.at(0) != 1 ||
            ...
            DLOG(WARNING)
                << "The shape of output tensor do not adapting with output oprand shape";
            const auto& target_shape =
                std::vector<uint32_t>{1, (uint32_t)out_shape.at(1),
                                      (uint32_t)out_shape.at(2)};
            out_oprand->data.at(b)->Reshape(target_shape);
          }
        }
        else {  // current shape size is 4
