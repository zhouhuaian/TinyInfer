...
void AttrLayer::set_weights(const std::vector<float>& weights) {
  ...
  const uint32_t weight_size = this->weights_.size();  // 权重总数
  uint32_t val_size = 0;  // 所有权重中的元素总数
  for (uint32_t i = 0; i < weight_size; ++i) {
    val_size += this->weights_.at(i)->size();
  }

  CHECK_EQ(val_size, elem_size);
  CHECK_EQ(elem_size % weight_size, 0);
  
  const uint32_t blob_size = elem_size / weight_size;  // 单个权重中的元素总数
  for (uint32_t i = 0; i < weight_size; ++i) {
    const uint32_t start = i * blob_size;
    const uint32_t end = start + blob_size;
    const auto& sub_vals = std::vector<float>{weights.begin() + start,
                                                weights.begin() + end};
    this->weights_.at(i)->Fill(sub_vals);
  }
}
...
void AttrLayer::set_bias(const std::vector<float>& bias) {
  ...
  for (uint32_t i = 0; i < bias_size; ++i) {
    const uint32_t start = i * blob_size;
    const uint32_t end = start + blob_size;
    const auto& sub_vals = std::vector<float>{bias.begin() + start,
                                                bias.begin() + end};
    this->bias_.at(i)->Fill(sub_vals);
  }
}
...
const std::vector<sftensor>& AttrLayer::bias() const {
  return this->bias_;
}
