...
InferStatus Layer::Forward() {
  ...
  // ! 准备Layer的输入Tensor——将所有来源的输入Tensor保存在一个同一个vector中
  ...
  // 分别检查输入输出空间是否为空
  CHECK(!in_datas.empty())
      << op->name << " operator input data is empty";
  CHECK(op->out_oprands != nullptr &&
        !op->out_oprands->data.empty())
      << op->name << " operator output data is empty";
  
  // Layer执行推理
  InferStatus status = this->Forward(in_datas, 
            op->out_oprands->data);
  return status;
}
...
void Layer::set_runtime_op(const srunop& op) {
  this->op_ = op;
}
