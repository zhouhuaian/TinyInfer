...
Cat::Cat(int dim) : NoAttrLayer("Cat"), dim_(dim) {}

InferStatus Cat::Forward(const std::vector<sftensor>& inputs, std::vector<sftensor>& outputs) {
  if (inputs.empty()) {
    LOG(ERROR) << "The input tensor array is empty";
    return InferStatus::InferFailedInputEmpty;
  }

  // ! Cat层要求输入Tensor和输出Tensor批次不相等
  ...
  // ! Cat目前只支持沿着Channel维度将Tensor拼接
  CHECK(dim_ == 1 || dim_ == -3) << "Cat dimension error";

  // ! 注意inputs保存了多个来源的输入Tensor，而Cat就是把多个来源的输入Tensor按Channel维度拼接
  ...
  for (uint32_t b = 0; b < out_batch; ++b) {
    ...
    for (uint32_t ib = b; ib < in_batch; ib += out_batch) {
      ...
      CHECK(input != nullptr && !input->empty()) 
          << "The " << ib << "th/st/nd input tensor is empty";
        
      CHECK(input->rows() == rows && input->cols() == cols)
          << "The " << ib << "th/st/nd input tensor dimension is wrong";
      ...
      CHECK(output->channels() == in_channels * packet_sz &&
          ...
          << "The " << b << "th/st/nd output tensor dimension is wrong";
...
ParseParamAttrStatus Cat::GetInstance(const srunop& op, slayer& cat) {
  if (op == nullptr) {
  ...
  const auto& dim_param = 
              dynamic_cast<RuntimeParamInt*>(params.at("dim"));
  if (dim_param == nullptr) {
    LOG(ERROR) << "Dim parameter is missing";
    return ParseParamAttrStatus::ParamMissingDim;
  }
  
  cat = std::make_shared<Cat>(dim_param->value);
  
  return ParseParamAttrStatus::ParamAttrParseSuccess;
}

LayerRegisterWrapper CatGetInstance("torch.cat", Cat::GetInstance);
