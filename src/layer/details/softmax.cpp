...
Softmax::Softmax(int dim) : NoAttrLayer("Softmax"), dim_(dim) {}

InferStatus Softmax::Forward(const std::vector<sftensor>& inputs, std::vector<sftensor>& outputs) {
  if (inputs.empty()) {
...
#pragma omp parallel for num_threads(batch)
  for (uint32_t b = 0; b < batch; ++b) {
    ...
    const uint32_t outer_sz = std::accumulate(
        raw_shape.begin(), raw_shape.begin() + dim, 
        1, std::multiplies<uint32_t>());
    const uint32_t inner_sz = std::accumulate(
        raw_shape.begin() + dim + 1, raw_shape.end(), 
        1, std::multiplies<uint32_t>());
...
#pragma omp parallel for collapse(2)  // 线程化下面两层循环
    for (uint32_t outer_idx = 0; outer_idx < outer_sz; ++outer_idx) {
      for (uint32_t inner_idx = 0; inner_idx < inner_sz; ++inner_idx) {
        ...
        // ! 计算dim轴上元素x_i的exp(x_i - max_val)值——添加偏移量为了避免计算指数值时溢出
        ...
        // 计算dim轴上元素x_i的softmax值——exp(x_i - max_val) / sum(exp(x_i - max_val))
        for (uint32_t axis_idx = 0; axis_idx < axis_sz; ++axis_idx) {
          uint32_t idx = POS_INDEX(outer_idx, inner_idx, axis_idx);
...
ParseParamAttrStatus Softmax::GetInstance(const srunop& op, slayer& softmax) {
  if (op == nullptr) {
...
LayerRegisterWrapper SoftmaxGetInstanceNN("nn.Softmax", Softmax::GetInstance);
LayerRegisterWrapper SoftmaxGetInstanceF("F.softmax", Softmax::GetInstance);
