...
InferStatus Convolution::Forward(const std::vector<sftensor>& inputs, std::vector<sftensor>& outputs) {
  if (inputs.empty()) {
  ...
  CHECK(kernel_ct > 0 && kernel_ct % groups_ == 0) 
      << "Kernel count error";
  ...
  for (uint32_t k = 0; k < kernel_ct; ++k) {
    ...
    CHECK(kernel->channels() == kernel_c && 
          ...
          << k << "th/st/nd kernel shape error";
    
    if (this->use_bias_) {
      const auto& bias = this->bias_.at(k);
      CHECK(bias != nullptr && !bias->empty()) 
          << k << "th/st/nd bias empty";
    }
  }
  ...
  std::vector<std::shared_ptr<arma::frowvec>> kernel_rows(kernel_ct);                 // 保存单分组下所有展平的kernels
  std::vector<std::vector<std::shared_ptr<arma::frowvec>>> gkernel_rows_vec(groups_,  // 保存多分组下所有展平的kernels 
  ...
  if (groups_ == 1) {
    for (uint32_t k = 0; k < kernel_ct; ++k) {
      ...
      kernel_rows.at(k) = 
          std::make_shared<arma::frowvec>(const_cast<float*>(kernel->raw_ptr()), 
              kernel_c * plane, false, true);
    }
  } 
  // 多分组
  else {
    for (uint32_t g = 0; g < groups_; ++g) {
      std::vector<std::shared_ptr<arma::frowvec>> gkernel_rows(gkernel_ct);  // 保存一个分组内展平的kernels
...
#pragma omp parallel for num_threads(batch)
  for (uint32_t b = 0; b < batch; ++b) {
    ...
    if (padding_h_ > 0 || padding_w_ > 0) {
      input_ = Pad(
          input, {padding_h_, padding_h_, padding_w_, padding_w_}, 0.f);
    } else {
      input_ = input;
    }
    ...
    CHECK(input_c % groups_ == 0) 
        << b << "th/st/nd input tensor channel error";    
    
    uint32_t ginput_c = input_c / groups_;  // 每组的特征图通道数
    // ! 这里也说明了分组卷积情况下，kernel的通道数已经和分组后特征图通道数一致！
    CHECK(ginput_c == kernel_c)
        << b << "th/st/nd input tensor grouped channel not equal to kernel channel";    
    ...
    CHECK(output_h > 0 && output_w > 0) 
        << b << "th/st/nd output shape error";
    
    auto& output = outputs.at(b);
    if (output == nullptr || output->empty()) {
      DLOG(ERROR) << b << "th/st/nd output tensor empty";
    ...
    CHECK(output->channels() == kernel_ct && output->rows() == output_h &&
        ...
        << b << "th/st/nd output tensor shape error";
    ...
    for (uint32_t g = 0; g < groups_; ++g) {
      ...
      for (uint32_t ic = 0; ic < ginput_c; ++ic) {
        const auto& in_channel = input_->slice(g * ginput_c + ic);  // 特征图单个通道
        ...
        for (uint32_t w = 0; w < input_w - kernel_w + 1; w += stride_w_) {
          for (uint32_t r = 0; r < input_h - kernel_h + 1; r += stride_h_) {
            float* in_mat_c_ptr = in_mat.colptr(in_mat_c_idx) + ic * plane;  // in_mat的列指针
            in_mat_c_idx += 1;
            // 按列展平一个kernel窗口内的元素
            for (uint32_t kw = 0; kw < kernel_w; ++kw) {
              const float* window_ptr = in_channel.colptr(w + kw) + r;  // 窗口内的列指针
...
ParseParamAttrStatus Convolution::GetInstance(const srunop& op, slayer& convolution) {
  if (!op) {
  ...
  const auto& bias = 
        dynamic_cast<RuntimeParamBool*>(params.at("bias"));
  if (!bias) {
    LOG(ERROR) << "Bias parameter missing";
    return ParseParamAttrStatus::ParamMissingBias;
  }
  ...
  const auto& groups = 
        dynamic_cast<RuntimeParamInt*>(params.at("groups"));
  if (!groups) {
    LOG(ERROR) << "Groups parameter missing";
    return ParseParamAttrStatus::ParamMissingGroups;
  }
  ...
  CHECK(kernel_size_val.size() == dims) 
      << "Kernel size parameter size error";

  CHECK(padding_val.size() == dims) 
      << "Padding parameter size error";

  CHECK(stride_val.size() == dims) 
      << "Stride parameter size error";

  // 创建convolution layer
  convolution = std::make_shared<Convolution>(
              out_channels->value, in_channels->value, 
              kernel_size_val.at(0), kernel_size_val.at(1),
              padding_val.at(0), padding_val.at(1), 
              stride_val.at(0), stride_val.at(1),
              groups->value, bias->value);
  ...
  CHECK(!weight->shape.empty()) 
      << "Weight attribute shape error";
...
LayerRegisterWrapper ConvGetInstance("nn.Conv2d", Convolution::GetInstance);
