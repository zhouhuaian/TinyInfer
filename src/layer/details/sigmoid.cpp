...
InferStatus Sigmoid::Forward(const std::vector<sftensor>& inputs, std::vector<sftensor>& outputs) {
  if (inputs.empty()) {
...
#pragma omp parallel for num_threads(batch)
  for (uint32_t b = 0; b < batch; ++b) {
    ...
    for ( ; i < size - 3; i += packet_size) {
      ...
      __m128 _out = _mm_div_ps(_one, _mm_add_ps(_one, exp_ps(_mm_sub_ps(_zero, _in))));
