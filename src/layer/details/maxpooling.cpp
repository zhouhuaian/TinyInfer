...
InferStatus MaxPooling::Forward(const std::vector<sftensor>& inputs, std::vector<sftensor>& outputs) {
  if (inputs.empty()) {
...
#pragma omp parallel for num_threads(batch)
  for (uint32_t b = 0; b < batch; ++b) {
    ...
    if (padding_h_ > 0 || padding_w_ > 0) {
      input_ = Pad(
          input, {padding_h_, padding_h_, padding_w_, padding_w_},
    ...
    uint32_t output_h = uint32_t(
        std::floor((input_h - kernel_h) / stride_h_ + 1));
    uint32_t output_w = uint32_t(
        std::floor((input_w - kernel_w) / stride_w_ + 1));
    
    CHECK(output_h > 0 && output_w > 0) 
        << "The " << b << "th/st/nd output shape error";
...
ParseParamAttrStatus MaxPooling::GetInstance(const srunop& op, slayer& maxpooling) {
  if (op == nullptr) {
  ...
  maxpooling = std::make_shared<MaxPooling>(padding.at(0), padding.at(1), 
...
LayerRegisterWrapper MaxPoolingGetInstance("nn.MaxPool2d", MaxPooling::GetInstance);
