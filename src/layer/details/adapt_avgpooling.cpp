...
InferStatus AdaptAvgPooling::Forward(const std::vector<sftensor>& inputs, std::vector<sftensor>& outputs) {
  if (inputs.empty()) {
  ...
  CHECK(this->output_h_ > 0 && this->output_w_ > 0) 
      << "Output shape error";
...
#pragma omp parallel for num_threads(batch)
  for (uint32_t b = 0; b < batch; ++b) {
    ...
    CHECK(stride_w > 0 && stride_h > 0)
        << "The stride of the " << b  << "th/st/nd input tensor is wrong";
    ...
    CHECK(kernel_h > 0 && kernel_w > 0)
        << "The kernel of the " << b  << "th/st/nd input tensor is wrong";
    ...
    for (uint32_t ic = 0; ic < input_c; ++ic) {
      ...
      for (uint32_t c = 0; c < input_w - kernel_w + 1; c += stride_w) {
        for (uint32_t r = 0; r < input_h - kernel_h + 1; r += stride_h) {
          ...
          *(out_col_ptr + int(r / stride_h)) = val_sum / float(kernel_h * kernel_w);
        }
      }
    }
  }
...
ParseParamAttrStatus AdaptAvgPooling::GetInstance(const srunop& op, slayer& adapt_avgpooling) {
  if (op == nullptr) {
  ...
  adapt_avgpooling = std::make_shared<AdaptAvgPooling>(out_hw.at(0), out_hw.at(1));
  
  return ParseParamAttrStatus::ParamAttrParseSuccess;
}

LayerRegisterWrapper AdaptAvgPoolingGetInstance(
                      "nn.AdaptiveAvgPool2d", AdaptAvgPooling::GetInstance);
