...
/* natural logarithm computed for 4 simultaneous float
   return NaN for x <= 0
*/
static TINY_FORCEINLINE v4sf log_ps(v4sf x) {
#ifdef USE_SSE2
  v4si emm0;
#else
  v2si mm0, mm1;
...
static TINY_FORCEINLINE v4sf exp_ps(v4sf x) {
...
/* evaluation of 4 sines slice onces, using only SSE1+MMX intrinsics so
   ...
   deliver full speed.
*/
static TINY_FORCEINLINE v4sf sin_ps(v4sf x) {   // any x
...
static TINY_FORCEINLINE v4sf cos_ps(v4sf x) {   // any x
...
static TINY_FORCEINLINE void sincos_ps(v4sf x, v4sf *s, v4sf *c) {
...
static TINY_FORCEINLINE __m128 tan_ps(__m128 x) {
  __m128 ysin, ycos;
...
static TINY_FORCEINLINE __m128 pow_ps(__m128 a, __m128 b) {
  // pow(x, m) = exp(m * log(x))
