...
InferStatus Linear::Forward(const std::vector<sftensor>& inputs, std::vector<sftensor>& outputs) {
  if (inputs.empty()) {
  ...
  arma::fmat weight(weight_tensor->data().memptr(), out_features_, 
                    in_features_, false, true); 
...
#pragma omp parallel for num_threads(batch)
  for (uint32_t b = 0; b < batch; ++b) {
    ...
    CHECK(in_shape.size() == 3 && in_shape.at(0) == 1 && 
        ...
        << "The " << b << "th/st/nd input tensor dimension is wrong";
    ...
    // 执行矩阵计算
    arma::fmat col_vec(input->data().memptr(), in_features_, 
                      in_dims, false, true);
...
ParseParamAttrStatus Linear::GetInstance(const srunop& op, slayer& linear) {
  if (op == nullptr) {
...
LayerRegisterWrapper LinearGetInstance("nn.Linear", Linear::GetInstance);
