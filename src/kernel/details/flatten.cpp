...
Flatten::Flatten(int start_dim, int end_dim)
    : NoAttrKernel("Flatten"), start_dim_(start_dim), end_dim_(end_dim) {}

InferStatus Flatten::Forward(const std::vector<sftensor> &inputs,
  ...
  CHECK(end_dim <= 2 && start_dim >= 0 && end_dim > start_dim)
      << "Flatten dimension error: "
      << "start_dim: " << start_dim_ << " end_dim: " << end_dim_;
  ...
  for (uint32_t b = 0; b < batch; ++b) {
    ...
    if (output == nullptr || output->empty()) {
      ...
      CHECK(input->size() == output->size())
          << "The " << b
          << " input and output tensor element size do not match";
...
ParseParamAttrStatus Flatten::Creator(const srunop &op, skernel &flatten) {
  ...
  const auto &end_dim = dynamic_cast<RuntimeParamInt *>(params.at("end_dim"));
  if (end_dim == nullptr) {
    LOG(ERROR) << "End dim parameter is missing";
    return ParseParamAttrStatus::ParamMissingDim;
  }
