...
public:
     ...
     */
    std::vector<sftensor> Forward(const std::vector<sftensor>& inputs, bool debug = false);
...
private:
     ...
     * @param op 计算图节点
     */
    static void InitOpInputs(const std::vector<pnnx::Operand*>& inputs, const srunop& op);
     ...
     * @param op 计算图节点
     */
    static void InitOpOutputs(const std::vector<pnnx::Operand*>& outputs, const srunop& op);
